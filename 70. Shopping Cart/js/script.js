...
check_out_btn.addEventListener("click", () => {
	if (confirm("Are you sure about removing the products in the shopping cart?")) clearCatItems();
});

countCounter.addEventListener("click", () => cartDOM.classList.toggle("active"));
...
addToCartBtn.forEach((btn) => {
	btn.addEventListener("click", () => {
		...
		if (!IsInCart) addItemToTheDom(product);
		else {
...
function increaseItem(inItem, product) {
	inItem.querySelector("[action='increase']").addEventListener("click", () => {
...
function decreaseItem(inItem, product) {
	inItem.querySelector("[action='decrease']").addEventListener("click", () => {
		cartItems.forEach((cartItem) => {
			if (cartItem.id === product.id) {
				if (cartItem.quantity > 1) inItem.querySelector(".product__quantity").innerText = --cartItem.quantity;
				else {
